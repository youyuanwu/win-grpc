...
int main()
{
    ...
    h_connect.connect(h_session.native_handle(), url.get_hostname().c_str(), url.get_port(), ec);
    if (ec)
    {
    ...
    c.SayHello(&request, [](boost::system::error_code ec, const helloworld::HelloReply * response)
               {
    if (ec) {
