...
void PrintSourceServerRouterApply(grpc_generator::Printer* printer,
                                  ...
                                  std::map<grpc::string, grpc::string>* vars) {
  printer->Print(*vars,
    "bool Service::HandleRequest(boost::system::error_code &ec, std::wstring const & url, std::string const & request, std::string & response) override {\n");
  printer->Indent();
  for (int i = 0; i < service->method_count(); ++i) {
