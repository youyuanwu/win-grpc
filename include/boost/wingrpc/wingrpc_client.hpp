...
namespace boost
{
    namespace wingrpc
    {
            ...
            _Out_ boost::system::error_code & ec)
        {
            ...
            h_session.set_option(WINHTTP_OPTION_ENABLE_HTTP_PROTOCOL, (PVOID) &f_enable_HTTP2, sizeof(f_enable_HTTP2), ec);
            if (ec)
            {
            ...
            h_session.set_option(WINHTTP_OPTION_REQUIRE_STREAM_END, (PVOID) &f_stream_end, sizeof(f_stream_end), ec);
            if (ec)
            {
            ...
            h_session.set_option(WINHTTP_OPTION_SECURE_PROTOCOLS, (PVOID) &f_tls_protocols, sizeof(f_tls_protocols), ec);
            if (ec)
            {
        ...
        // TODO: test this
        inline void parse_trailers(_In_ std::wstring const & trailer_str, _Out_ std::map<std::wstring, std::wstring> & out)
        {
        ...
        inline bool get_trailer_val(_In_ std::map<std::wstring, std::wstring> & trailers, _In_ std::wstring const & key, _Out_ std::wstring & val)
        {
        ...
        inline bool get_grpc_status(_In_ std::map<std::wstring, std::wstring> & trailers, _Out_ std::wstring & code)
        {
        ...
        // message is optional.
        inline bool
        get_grpc_message(_In_ std::map<std::wstring, std::wstring> & trailers, _Out_ std::wstring & message)
        {
        ...
        void client_exec(
            std::shared_ptr<request_holder<Executor>>
                req,
                ...
                token)
        {
            ...
            winnet::winhttp::async_exec(
                ...
                h_connect,
                req->h_request_,
                req->dynamic_body_buff_,
                [req, token = std::move(token)](boost::system::error_code ec, std::size_t) mutable
                {
